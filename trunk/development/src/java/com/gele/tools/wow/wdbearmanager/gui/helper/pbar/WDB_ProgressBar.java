 ...
 *
 * @version   $Id: WDB_ProgressBar.java 195 2005-10-30 16:44:25Z gleibrock $
 *
 * License:
 ...
 * $Author: gleibrock $ 
 * $HeadURL: svn://painkiller/wdb-manager/development/src/java/com/gele/tools/wow/wdbearmanager/gui/helper/pbar/WDB_ProgressBar.java $
 * $Rev: 195 $
 */
public class WDB_ProgressBar extends Thread {
  // Logging with Log4J
  Logger myLogger = Logger.getLogger(WDB_ProgressBar.class);
  ...
  private WDBMgr_Task_I myTask = null;
  ...
  public WDB_ProgressBar(WDBMgr_Caller_I parCaller, WDBMgr_Task_I parTask,
      String parTitle) {
   ...
   */
  private HelperThread myHT = null;

  public void run() {
    this.myHT = new HelperThread(this.myTask);
    this.myHT.start();
    ...
    this.timer = new Timer(ONE_SECOND, new ActionListener() {
      public void actionPerformed(ActionEvent evt) {
        ...
        WDB_ProgressBar.this.progressBar.setValue(WDB_ProgressBar.this.myHT
            .getCurrent());
        String s = WDB_ProgressBar.this.myHT.getMessage();
        if (s != null) {
          WDB_ProgressBar.this.taskOutput.setText(s);
          /*
        ...
        if (WDB_ProgressBar.this.myHT.isDone()) {
          //Toolkit.getDefaultToolkit().beep();
          WDB_ProgressBar.this.timer.stop();
          WDB_ProgressBar.this.myPanel.setCursor(null); //turn off the wait cursor
          //progressBar.setValue(progressBar.getMinimum());
          WDB_ProgressBar.this.frame
              .setDefaultCloseOperation(WindowConstants.HIDE_ON_CLOSE);
          WDB_ProgressBar.this.myCaller.taskFinished(WDB_ProgressBar.this.myHT
              .getResult(), new Date().getTime()
              - WDB_ProgressBar.this.startDate.getTime());
          WDB_ProgressBar.this.frame.setVisible(false);
        }
        if (WDB_ProgressBar.this.myHT.getException() != null) {
          WDB_ProgressBar.this.timer.stop();
          WDB_ProgressBar.this.myHT.getMessage();
          WDB_ProgressBar.this.frame.setVisible(false);
          WDB_ProgressBar.this.myCaller.taskError(WDB_ProgressBar.this.myHT
              .getException());
        } else {
        }
      }
